...
class BanditNet(nn.Module):
    ...
    def forward(self, data, cache):
        ...
        return attn_weights1, attn_weights1, updated_cache, new_predict
    ...
    for episode in range(num_episodes):
        ...
        while data != None:
            ...
            if random.random() < epsilon:  # 以 ε 的概率进行随机探索
                action = torch.randint(0, q_values1.size(1), (q_values1.size(0),))  # 随机选择动作
            else:
            ...
            reward1, reward2, data, cache = env.step(action=action, newcache=cache)
