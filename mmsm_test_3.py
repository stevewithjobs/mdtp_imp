...
import time
import yaml
from mmsm_train_3 import BanditNet, MultiArmedBanditEnv 
...
parser.add_argument('--rlsave', type=str, default='./mmsm_model/mmsm_200.pth', help='save path')
...
if __name__ == '__main__':
    ...
    while data != None:
        ...
        t1 = time.time()
        with torch.no_grad():
            new_predict, new_cache = bandit_Net(data, cache)  # Q值估计
        t2 = time.time()
        # action = torch.argmax(q_values1, dim=1)  # 选择最大Q值对应的动作  
        if t > 1:
            tt += t2 - t1  
        # if t < 4:
