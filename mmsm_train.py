...
class MultiArmedBanditEnv:
    ...
    def _getreward(self, action, newcache):
        ...
        min_indices = torch.argmin(cache_loss, dim=1)
        return min_indices
...
class MultiTensorAttention(nn.Module):
    def __init__(self, batch_size):
        ...
        self.query_proj = nn.Sequential(
            # 第一层：通道数从 4 -> 16
            nn.Conv2d(in_channels=4, out_channels=16, kernel_size=3, stride=2, padding=0),
            nn.ReLU(),
            nn.BatchNorm2d(16),
            
            # 第二层：通道数从 16 -> 4
            nn.Conv2d(in_channels=16, out_channels=4, kernel_size=3, stride=2, padding=0),
            nn.ReLU(),
            nn.BatchNorm2d(4)
        )
...
class BanditNet(nn.Module):
    ...
    def forward(self, data, cache, timesnet_cache):
        ...
        results = updated_cache[:, :, 0, :, 0].reshape(self.batch_size, 4, -1)
...
if __name__ == '__main__':
    ...
    for episode in range(num_episodes):
        ...
        while data != None:
            ...
            # 在环境中执行动作
            final_pred = cache[:,:,0,:,:] * q_values.unsqueeze(-1).unsqueeze(-1)
            ...
            loss = nn.CrossEntropyLoss()(q_values, target)
            # print(f"Loss1: {loss:.4f}")
            loss += bike_start_metrics * 100
