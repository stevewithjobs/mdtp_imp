...
class SMoERouting(torch.autograd.Function):
    ...
    def forward(ctx: Any,
        ...
        vals, indices = routing_weights.topk(k=smoe_config.out_planes, dim=2)
        ctx.indices = indices
        ctx.save_for_backward(experts, routing_weights)

        routing_map = torch.zeros_like(routing_weights).scatter_(
            dim=2, index=indices, src=vals)
        ctx.mark_non_differentiable(routing_map)  # Will not need gradients.
        ctx.mark_non_differentiable(indices)
        extend_indices = indices.unsqueeze(-1).expand(-1, -1, -1, -1, 32)
        if smoe_config.unweighted:
            selected = torch.gather(experts, dim=1, index=extend_indices)
        else:
            extend_routing_map = routing_map.unsqueeze(-1)
            scaled_experts = experts * extend_routing_map
            selected = torch.gather(scaled_experts, dim=1, index=extend_indices)
    ...
    def backward(ctx: Any,
        ...
        if ctx.save_module and smoe_config.save_error_signal:
            ...
            tmp = tmp.mean(-1)
            tmp = tmp.squeeze(-1)
            ctx.save_module.saved_error_signal = tmp
            ...
            if not smoe_config.unweighted:
                ...
                extended_indices = ctx.indices.unsqueeze(-1)
                selected_experts = torch.gather(experts, dim=1, index=extended_indices)
                tmp = grad_selected * selected_experts
                tmp = tmp.mean(-1)
                tmp = tmp.squeeze(-1)
                ctx.save_module.saved_error_signal = tmp
        ...
        # print("grad_selected:", grad_selected.size())
        # print("ctx.indices:", ctx.indices.size())
        # print("experts:", experts.size())

        extended_indices = ctx.indices.unsqueeze(-1) 
        scattered_grads = torch.zeros(experts.size(), dtype=grad_selected.dtype,
                                      device=grad_selected.device).scatter_(
            dim=2, index=extended_indices, src=grad_selected)

        if ctx.needs_input_grad[0]:
            if smoe_config.unweighted:
                grad_experts = scattered_grads
            else:
                extend_routing_weights = routing_weights.unsqueeze(-1)
                grad_experts = scattered_grads * extend_routing_weights
        ...
        num_experts = scattered_grads.size(2)
        ...
        if ((smoe_config.smooth_gate_error or smoe_config.dampen_expert_error)
            ...
            if smoe_config.dampen_expert_error:
                ...
                damping = torch.ones_like(scattered_grads).scatter_(
                    dim=1, index=extended_indices, src=scaling_factors)
